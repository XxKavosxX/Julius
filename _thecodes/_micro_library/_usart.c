...
#include "../includes/_usart.h"
 ...
 void usart_send(unsigned char dado){
	 ...
	 //POSS��VEL PROBLEMA NESTE LA��O
	 //Espera limpeza do registrador de transmiss��o
     while (!(UCSR0A &(1<<UDRE0)));
     UDR0 = dado;
 }
 unsigned char usart_receive(){
     //Espera o dado recebido
     while (!(UCSR0A &(1<<RXC0)) );
     return UDR0;
 }
 void usart_write(char *c){
     for (; *c!=0; c++) usart_send(*c);
 }
 void usart_write_flash(const char *c){
     for (; pgm_read_byte(&(*c))!=0;c++) usart_send(pgm_read_byte(&(*c)));
 }
